...
public:
  ...
  virtual int Connect (const Address &address);      // Setup endpoint and call OnAction() to connect
  virtual int Listen (void);  // Verify the socket is in a correct state and call OnAction() to listen
...
Actions_t OnEvent (Events_t e);


bool OnAction (Actions_t a);


bool OnAction_PacketInfo (Actions_t a, Ptr<Packet> p,
   ...
   * \param toAddress the destination address
   */
  void Notify_sender (Ptr<Packet> packet, const TcpHeader& tcpHeader,
                       const Address& fromAddress, const Address& toAddress);
...
protected:
  ...
  TracedValue<States_t> m_state;         //!< TCP state
